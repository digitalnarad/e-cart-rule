...
class RuleEngineService {
  ...
  async initialize(rules) {
    ...
    this.addOrderItemsFact("order_category_ids", (cart) => [
      ...new Set(cart.order_items.map((item) => item.category_id.toString())),
    ]);

    this.addOrderItemsFact("order_sub_category_ids", (cart) => [
      ...new Set(
        cart.order_items.map((item) => item.sub_category_id.toString())
      ),
    ]);

    this.addOrderItemsFact("order_product_prices", (cart) =>
      cart.order_items.map((item) => item.price)
    );

    this.addOrderItemsFact("order_product_quantities", (cart) =>
      cart.order_items.map((item) => item.quantity)
    );
  }
