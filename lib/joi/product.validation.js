...
const updateProduct = Joi.object({
  ...
  actual_price: Joi.number().min(0).messages({
    "number.base": "Actual price must be a number",
    "number.min": "Actual price cannot be negative",
  }),

  product_company: Joi.string().messages({
    "string.empty": "Product company is not alow empty",
  }),

  offer_price: Joi.number().min(0).messages({
    "number.base": "Offer price must be a number",
    "number.min": "Offer price cannot be negative",
  }),
  ...
  category_id: Joi.string().custom(objectIdValidator, "ObjectId validation"),

  sub_category_id: Joi.string().custom(
    objectIdValidator,
    "ObjectId validation"
  ),
